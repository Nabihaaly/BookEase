...
const EditAppointmentModal = ({ isOpen, appointment, onClose }) => {
  ...
  const { fetchServices, services } = useContext(ServicesContext);
  ...
  const [time, setTime] = useState(
    appointment?.appointmentDateTime
      ? appointment.appointmentDateTime.split("T")[1].slice(0, 5)
      : ""
  );
  // "2025-09-30T15:30:00"
  // 15:30 slice krky
  ...
  useEffect(()=>{
    const loadService= async ()=>{
      try{
        ...
        await fetchServices(); // this should update 'services' state from context or props

      const matchedService = services?.find(s => s.id === appointment.serviceID);
      if (matchedService) {
        setSelectedService(matchedService);
      } else {
        console.warn("No matching service found for ID:", appointment.serviceID);
      }
      }catch (err) {
  ...
  const getAvailableDayNumbers = () => {
    ...
    return selectedService?.daysOfWeek?.map((day) => dayMap[day]) || [];
  };
  ...
  const getSlots = async () => {
    try {
      const res = await api.get(
        `/user/service/${appointment.serviceID}/slots?date=${date}`
      );
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full max-h-[85vh] flex flex-col shadow-2xl">
        ...
        <div className="px-6 pb-6 space-y-6 flex-1 overflow-y-auto scroll-smooth">
          ...
          <div>
           ...
           {/* Available Days */}
          {selectedService?.daysOfWeek?.length > 0 ? (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              <p className="text-sm text-blue-900">
                <span className="font-semibold">Available on: </span>
                {selectedService.daysOfWeek.join(", ")}
              </p>
            </div>
          ) : (
            <p className="text-sm text-gray-500 italic">
              No available days configured for this service.
            </p>
          )}
            ...
            <input
              ...
              onInput={(e) => {
                if (!isDateAvailable(e.target.value)) {
                  alert(
                    `Service is not available on this day. Available days: ${selectedService?.daysOfWeek?.join(", ") || "None"}`
                  );
                  e.target.value = date;
                }
              }}
        ...
        <div className="px-3 pt-2 pb-3 bg-gray-50 border-t border-gray-100 flex-shrink-0">
          ...
          <div className=" pt-2 border-t border-gray-200">
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-semibold text-purple-600">
                Total: Rs.{appointment.servicePrice}
              </span>
            </div>
          </div>
        </div>
